...
import EMPTY_BYTE_ARRAY
import EMPTY_HEADERS
import android.util.Log
import assertThreadDoesntHoldLock
import closeQuietly
import com.docwei.okhttp.concurrent.TaskRunner
import com.docwei.okhttp.http2.Settings.Companion.DEFAULT_INITIAL_WINDOW_SIZE
import ignoreIoExceptions
import notifyAll
...
class Http2Connection internal constructor(builder: Builder) : Closeable {
     ...
     */
    @Throws(IOException::class)
    fun newStream(requestHeaders: List<Header>, out: Boolean): Http2Stream {
        return newStream(0, requestHeaders, out)
    }

    @Throws(IOException::class)
    private fun newStream(associatedStreamId: Int, requestHeaders: List<Header>, out: Boolean): Http2Stream {
        val outFinished = !out
    ...
    fun start(sendConnectionPreface: Boolean = true) {
        if (sendConnectionPreface) {
            ...
            Log.e("okhttp","windowSize-->${windowSize}")

            if (windowSize != DEFAULT_INITIAL_WINDOW_SIZE) {
                writer.windowUpdate(0, (windowSize - DEFAULT_INITIAL_WINDOW_SIZE).toLong())
                Log.e("okhttp","(windowSize - Settings.DEFAULT_INITIAL_WINDOW_SIZE).toLong()-->${(windowSize - DEFAULT_INITIAL_WINDOW_SIZE).toLong()}")
            }
        }
